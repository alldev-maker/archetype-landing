const BlockWithImage = ({ heading, contents, image, background }) => {
  return <section className={`section-gap ${background ? 'purple-gradient' : ''}`}>
    <div className="container">
      <div className={`flex items-center flex-wrap px-5 lg:px-14 ${background ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className={`w-full flex justify-center mt-5 sm:w-1/2 sm:mt-0 ${background ? 'md:justify-start' : 'md:justify-end'}`}>
